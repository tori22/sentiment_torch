...
from models import LSTM, IAN, RAM
    ...
    def _evaluate_acc(self):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(self.test_data_loader):
                ...
                test_out = t_outputs.cpu().data.numpy()
                n_test_correct += (torch.argmax(t_outputs, -1) == t_targets).sum().item()
                n_test_total += len(t_outputs)

            for t_batch, t_sample_batched in enumerate(self.final_data_loader):
                t_inputs = [t_sample_batched[col].to(opt.device) for col in self.opt.inputs_cols]
                t_outputs = self.model(t_inputs)
                pred = torch.argmax(t_outputs,-1)
                final = pred.cpu().data.numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='ccf', type=str, help='ccf')
